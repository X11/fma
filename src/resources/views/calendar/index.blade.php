...
<section class="section">
    <div class="container calendar is-{{ $overview_container }}">
    ...
    <div class="container calendar is-{{ $overview_container }}">
        <div class="columns is-multiline">
            @foreach($dates as $week)
                ...
                <div style="order: {{$order}};" class="column is-3 calendar-column {{ $day == $today ? 'is-active' : '' }}">
                    <div class="heading">
                        <h2>{{ Carbon\Carbon::parse($day)->format('D, j M') }}</h2>
                    </div>
                    <ul class="calendar-list">
                        @foreach($episodes->get($day, collect([]))->sortBy('serie_name') as $episode)
                        <li class="calendar-item {{ in_array($episode->serie->id, $watching_ids) ? 'is-watching' : ''}} {{ $episode->episodeSeason == 1 && $episode->episodeNumber == 1 ? 'is-premier' : '' }} {{ $episode->episodeNumber == 1 ? 'is-returning' : ''}}">
                            <a href="{{ $episode->url }}">
                                <label class="date">
